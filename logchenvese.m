...
function seg = logchenvese(I,mask,num_iter,mu,method)
...
%CHANGED
[ix,iy]=imgradientxy(P);
%gradi=[ix;iy];
Master=zeros(200,200,2,2);
%TFG=imgaussfilt(gradi*gradi',2);
 for i=1:200
    for j=1:200
          grad=[ix(i,j)*ix(i,j) ix(i,j)*iy(i,j); ix(i,j)*iy(i,j) iy(i,j)*iy(i,j)];
          Master(i,j,:,:)=imgaussfilt(grad,2);
%         TF=[ix(i,j)^2 ix(i,j)*iy(i,j);ix(i,j)*iy(i,j)  iy(i,j)^2];
%         TFG=imgaussfilt(TF,2);
%         
    end
 end
%%
          ...
          for n=1:num_iter
              ...
              for i=1:layer
                 % L = im2double(P(:,:,i)); % get one image component
                  ...
                  for h=1:200
                      for t=1:200
                          temp=Master(h,t,:,:)*hv(h,t);
                          sum1=sum1+[temp(:,:,1,1) temp(:,:,1,2);temp(:,:,2,1) temp(:,:,2,2)];
                      end
                  end
                  for h=1:200
                      for t=1:200
                          temp=Master(h,t,:,:)*(1-hv(h,t));
                          sum2=sum2+[temp(:,:,1,1) temp(:,:,1,2);temp(:,:,2,1) temp(:,:,2,2)];
                      end
                  end
                  %c1 = sum(sum(L.*Heaviside(phi0)))/(length(inidx)+eps); % average inside of Phi0
                  c1=sum1/(length(inidx)+eps);
                  %c2 = sum(sum(L.*(1-Heaviside(phi0))))/(length(outidx)+eps);
                  c2=sum2/(length(outidx)+eps);% verage outside of Phi0
                   for h=1:200
                      for t=1:200
                         temp=Master(h,t,:,:);
                         k1=[temp(:,:,1,1) temp(:,:,1,2);temp(:,:,2,1) temp(:,:,2,2)];
                         e1=eig(k1,"matrix");
                         M = min(e1);
                         e1=e1-M+eps;
                         e1(e1<eps)=eps;
                         ...
                         Li1(isnan(Li1))=eps;
                         Li2(isnan(Li2))=eps;
                         dis1=norm((Li1-Li2),'fro')^2;
                         if(dis1==inf)
                             dis1=eps;
                         end
                         
                         L1(h,t)=abs(dis1);
                        % k2=[temp(:,:,1,1) temp(:,:,1,2);temp(:,:,2,1) temp(:,:,2,2)];
%                          e1=eig(k2,"matrix");
%                          M = min(e1);
%                          e1=e1-M+eps;
                         e2=eig(c2,"matrix");
                  ...
                  force_image=-(L1).^2+(L2).^2+force_image; 
